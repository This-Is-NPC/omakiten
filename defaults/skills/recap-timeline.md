---
name: Recap timeline
description: Build a single-project chronological recap from project-scoped comments (all kinds) plus tasks moved to done within a window, rendered read-only.
schema_version: 2
role_affinity:
  - Scribe
---
A recap is a historical readout for one project — a what-happened view rather than a what-next view. The skill assembles raw entries from existing comments and renders them; it does not synthesise new content or persist anything.
...
1. `comments_list` with `scope=project` for the resolved project, ordered by `created_at desc`, `since` mapped from `--since`, and optionally filtered by `kind` (one call per kind when `--kinds` lists several). Page through results until the window is exhausted; do not silently cap.
...
3. Do not include comments from other projects, even if shared context exists — recap is single-project by design.
...
- Tasks moved to done are rendered as their own group labelled clearly (not folded into comment kinds).
...
- Preserve comment titles verbatim. Truncate bodies to a short preview (target ~200 chars) with a `…` marker; do not paraphrase.
...
- **Read-only.** No `comments_add`, no task mutation. The only side effect is the rendered output written to the command surface.
