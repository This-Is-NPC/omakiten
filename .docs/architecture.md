...
| `internal/configstore/` | Right-side adapter that wraps `internal/config` I/O behind the `app.BundleStore` and `app.EntityFileWriter` ports; carries the discovered repo-local `.omakiten/` path so every BundleStore.LoadBundle through this runtime sees the 4-layer overlay |
...
- **4-layer config resolution**: Bundle loading merges kit defaults → user-global default → user-global custom → repo-local `.omakiten/` (discovered by `config.FindRepoLocal` walk-up). Higher layers override by slug (entity files) or identity key (wiring sections — workflows by `key`, personas/projects by `slug`, mcp_commands by map key, config by nested deep-merge). `*_disabled` lists in the repo-local yaml subtract inherited entries from both wiring slots and auto-loaded entity sets. The repo-local layer is read-only at this revision; only the user-global custom layer is editable through the bundle editor. Sources: `internal/config/repo_local.go`, `internal/config/wiring_merge.go`, `internal/config/entity_loader.go`.
...
- **Hot-reload of the active config bundle**: the TUI Settings → Config picker re-imports a chosen preset in place through `Model.reloadBundle` — it calls `app.ConfigService.Import` (which returns an instance-scoped `*domain.EnumRegistry`), repoints `BundleEditor.SetPath`, swaps `WorkflowService.SetRegistry`, refreshes theme/styles/markdown/notifications/token-badge, and re-queries the task snapshot. `paths.SetActiveConfig` is written only after the import succeeds so a validator rejection on the new bundle leaves the on-disk `.active` pointing at the previous (working) profile. A successful swap emits `bundle.swapped` with the orphan-preview payload; when orphans exist the model stashes the previous config path and the hook engine paints the `kitten_orphan_migration` prompt — esc on that prompt re-imports the previous bundle so the user is never left on a workflow they did not confirm (`internal/tui/reload_bundle.go`, `internal/tui/settings_picker.go`).
