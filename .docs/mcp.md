...
| `comments.edit` | Rewrites a comment's body and, optionally, its `title`/`kind`/`pinned`, and replaces its tags. Subject to bucket `permissions.comment.edit` (inherits from `permissions.task.edit` when no comment block is declared). |
