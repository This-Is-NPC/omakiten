...
4. **CHANGELOG is generated by release-please.** Detail lives in the PR body or in the release-please PR, never in a hand-written Unreleased section.
...
Renders in any markdown viewer. If link checking is added to the local gate, this is the pattern it should validate.
...
Runs as part of `mise run check` and the local pre-push merge gate. Fails if regenerating would produce a non-empty diff. Failure message names the drifting files; the fix is to run `mise run docs:refresh` locally and commit the result.
...
3. Run `mise run docs:refresh` — generated catalogs and marker blocks update in-place.
4. Run `mise run check` — lint, vet, tests, and the docs-drift gate must all stay green.

Two hand-edited files total: the entity and the preset yaml. Everything else regenerates; release notes come from the PR / release-please flow.
...
3. Run `mise run check` — the parity test (`internal/config/language_pack_parity_test.go`) catches missing or extra keys.
