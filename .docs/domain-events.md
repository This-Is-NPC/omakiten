...
| `bundle.imported`     | system (entity_id=0)    | `Store.ImportBundle` after `providers.Swap` rotates the in-memory snapshot | `{path, hash, workflow_key, workflow_count, persona_count, skill_count, law_count, template_count}` | system            | true        |
