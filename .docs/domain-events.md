...
| `task.removed`        | system (project-scoped) | `Store.HardDeleteTask`                                        | `{task_id, title, description, priority, bucket_key, state}` (`priority` is the integer id) | cli / mcp / tui   | true        |
...
| `task.migrated`       | task                    | `Store.RebindOrphanedTasks` (workflow swap rebind)            | `{from, to, reason:"workflow_swap"}`                           | tui / cli / mcp   | true        |
...
| `guard.violated`      | task                    | Every `domain.ErrGuardViolation` return path (transitions, operation guards, permission denials) — comment-scoped denials still attach the event to the parent task; the `comment_id` rides in `payload.target` | `{operation, rule, hint, target, attempted_by}`               | cli / mcp / tui   | true        |
| `error.recorded`      | error                   | `ErrorService.Record`                                         | `{tags, has_context}`                                          | mcp (typical)     | true        |
| `error.searched`      | error (entity_id=0)     | `SearchService.Search` (unified FTS5 across tasks / comments / errors / solutions / context entries) | `{query, entity_types, result_count, unified}`            | mcp (typical)     | true        |
...
| `bundle.imported`     | system (entity_id=0)    | `agentruntime.buildProjectRuntime` composes the payload and calls `Store.RecordEntityEvent` **before** `config.BuildSnapshot` materialises the per-project Snapshot — the import is the source of the audit row, not the snapshot materialisation | `{path, hash, workflow_key, workflow_count, persona_count, skill_count, law_count, template_count}` | system            | true        |
