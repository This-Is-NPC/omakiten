...
// knownEventsForTest is the closed event_type set the hook validator
// tests assert against. We seed every domain constant the test bodies
// reference so the only "unknown" path is the deliberate one in
// TestValidateHooksRejectsUnknownEventType.
func knownEventsForTest() map[string]struct{} {
	return map[string]struct{}{
		domain.EventTypeGuardViolated: {},
...
func TestValidateHooksHappyPath(t *testing.T) {
	...
	if err := ValidateHooks(specs, knownEventsForTest(), func(name string) bool { return name == "exec" || name == "noop" }, nil); err != nil {
...
func TestValidateHooksRejectsUnknownEventType(t *testing.T) {
	...
	err := ValidateHooks(specs, knownEventsForTest(), func(string) bool { return true }, nil)
...
func TestValidateHooksRejectsUnknownAction(t *testing.T) {
	...
	err := ValidateHooks(specs, knownEventsForTest(), func(name string) bool { return name == "exec" }, nil)
...
func TestValidateHooks_notificationHookMessageFromHookOK(t *testing.T) {
	...
	if err := ValidateHooks(specs, knownEventsForTest(), func(string) bool { return true }, notifications); err != nil {
...
func TestValidateHooks_notificationHookNoMessageAnywhere(t *testing.T) {
	...
	notifications := map[string]Notification{"kit": {Name: "kit"}}
	err := ValidateHooks(specs, knownEventsForTest(), func(string) bool { return true }, notifications)
