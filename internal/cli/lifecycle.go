	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runJSON(cmd, func(ctx context.Context) (any, error) {
				...
				event, err := app.NewTaskServiceFromStore(rt.store).Delete(ctx, project, taskID)
				if err != nil {
					return nil, err
				}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runJSON(cmd, func(ctx context.Context) (any, error) {
				...
				task, _, err := app.NewTaskServiceFromStore(rt.store).Archive(ctx, project, taskID)
				if err != nil {
					return nil, err
				}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runJSON(cmd, func(ctx context.Context) (any, error) {
				...
				task, _, err := app.NewTaskServiceFromStore(rt.store).Unarchive(ctx, project, taskID)
				if err != nil {
					return nil, err
				}
				return map[string]any{"project": project, "task": task}, nil
