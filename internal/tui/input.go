...
func (m *Model) submitInput() {
	...
	case modeMove:
		...
		savedTask, err = app.NewTaskService(m.repos.Tasks, m.repos.Workflow, nil).Move(m.ctx, m.project, task.ID, input)
...
func (m *Model) moveSelectedToColumn(targetColIdx int) {
	...
	if _, err := app.NewTaskService(m.repos.Tasks, m.repos.Workflow, nil).Move(m.ctx, m.project, task.ID, target.Key); err != nil {
		m.status = err.Error()
		m.moveMode = false
		return
	}
