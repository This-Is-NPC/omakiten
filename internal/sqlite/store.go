...
// (tasks.go, comments.go, ...) so this file stays small and focused on
// lifecycle: opening, closing, and bringing the schema up to date. ImportBundle
// also lives here because the entire schema-shaped config snapshot is one
// transactional concern that has no natural home elsewhere.
type Store struct {
	db *sql.DB
}
