#!/usr/bin/env python3
"""Apply remaining catalog translations to non-en language packs."""
...
DATA_FILE = Path(__file__).with_name("i18n_remaining_translations.json")
...
def main() -> int:
    ...
    for code in sorted(data):
        print(f"applying {code} ({len(data[code])} keys)...")
        apply_locale(code, data[code])
    print("done")
    return 0
