...
- Project-level reference (auto-generated by `/document`): `.docs/internal/architecture.md`, `.docs/_generated/requirements.md`.
- When changing behavior covered by an existing guide, update the guide in the same PR.

### CHANGELOG

`CHANGELOG.md` is generated by `release-please` (`release-please-config.json`, `.release-please-manifest.json`) from conventional-commits subjects on merge. Do not hand-edit version headings or add an `## Unreleased` section in feature PRs — release-please ignores hand-written prose and the orphan section piles up. Write your context-rich prose in the PR body instead; the version section emitted by release-please already links to the merging PR. If a richer changelog entry is needed, edit it inside the release-please-prepared PR (the one titled `chore(master): release <next-version>`) before merging — that PR is the only place hand edits survive.
