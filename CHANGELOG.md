...
* **workflow presets:** official `omakase`, `izakaya`, `kaiseki`, and `shokunin` starter configs now ship as flat YAML files under `defaults/config/`. `okt init --preset <name>` copies the selected preset to `.omakiten/config/omakiten.yaml` with `--preset-force` for explicit overwrite, and `okt config presets` lists the preset menu.
* **notifications:** configurable notification cards now support hook-level short messages plus optional `tab` detail pages, shared footer rendering with `footer_position`, and timeout dismissal that can still be closed manually. The default kit wires common workflow events to reusable notification recipes while keeping event-specific copy in `defaults/omakiten.yaml`.
